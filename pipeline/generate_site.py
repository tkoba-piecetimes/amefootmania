# -*- coding: utf-8 -*-
"""data/leagues/ の正規化JSONから静的サイト「アメフトマニア」（site/）を生成する。
rugbymania（C:\\Users\\tatsu\\claudecode\\rugbymania）の pipeline/generate_site.py の
アーキテクチャを移植したもの。主な相違点は末尾のコメント参照。

URL構造:
  site/index.html                     ポータルトップ（TOP8・BIG8の最新結果）
...
def winner_team(m):
    """勝者チーム名（引き分け・未実施はNone）。星取表の勝敗記号（○●）から判定するため、
    同点でもタイブレークで決着したカードは正しく勝者を返す（下記コメント参照）。"""
    if m["winner"] == "home":
        return m["home"]
    if m["winner"] == "away":
        return m["away"]
...
def match_headline(m):
    ...
    return f'【{m["category"]}】{w} 勝利　{m["home"]} {hs}-{as_} {m["away"]}'


def match_report(m, standings, league_name):
    d = date_jp(m["date"], with_year=True) if m["date"] else "日程未定"
    if m["status"] != "played":
        return f'{d}、{league_name}の{m["home"]}対{m["away"]}が行われる予定です。'
    hs, as_ = m["home_score"], m["away_score"]
    base = f'{d}、{league_name}の{m["home"]}対{m["away"]}が行われました。'
    w = winner_team(m)
    if w is None:
        result = f'試合は{hs}-{as_}の引き分けに終わりました。'
    elif hs == as_:
        # 表示スコアは同点だがタイブレークで決着したカード。
        result = f'試合は{hs}-{as_}のままタイブレークに突入し、{w}が勝利しました。'
    elif w == m["home"]:
        result = f'試合は{w}が{hs}-{as_}で勝利しました。'
    else:
        result = f'試合は{w}が{as_}-{hs}で勝利しました。'
    ctx = ""
    for e in standings.get(m["category"], []):
        if w and e["team"] == w and e.get("rank") is not None:
...
def match_row(m, L, league_label=None, league_code=None, with_venue=False):
    ...
    venue_cell = f'<td class="venue">{escape(m["venue"])}</td>' if with_venue else ""
    return (f'<tr><td>{d}</td>{lg_cell}'
            f'<td><a href="{link}">{escape(m["home"])} vs {escape(m["away"])}</a></td>'
            f'<td class="score">{score_str(m)}</td>'
            f'{venue_cell}</tr>')


def match_table(rows, with_league=False, with_venue=False):
    lg_th = "<th>リーグ</th>" if with_league else ""
    venue_th = "<th>会場</th>" if with_venue else ""
    return (f'<div class="tbl"><table><thead><tr><th>日付</th>{lg_th}'
            f'<th>対戦</th><th>スコア</th>{venue_th}</tr></thead>'
            f'<tbody>{rows}</tbody></table></div>')


def match_rows_table(ms, L, league_label=None, league_code=None):
    """試合リストからmatch_table一式を組み立てる。会場列は当季の「今後の試合」
    （result_date/由来）にしかデータがないため、リスト内に会場情報がある場合だけ
    列を出す（行と見出しの列数を必ず一致させる）。"""
    with_venue = any(m.get("venue") for m in ms)
    rows = "".join(match_row(m, L, league_label=league_label, league_code=league_code,
                             with_venue=with_venue) for m in ms)
    return match_table(rows, with_league=bool(league_label), with_venue=with_venue)
...
def standings_table(block, entries, L):
    rows = "".join(
        ...
        for e in entries)
    return ('<div class="tbl"><table><thead><tr><th>順位</th><th>チーム</th><th>勝ち点</th>'
            '<th>試合</th><th>勝-敗</th><th>得失点差</th></tr></thead>'
            f'<tbody>{rows}</tbody></table></div>')
...
def h2h_section(m, matches_by_year):
    ...
    rows = "".join(
        ...
        for y, x in pair[:6])
    record = f'{wins}勝{losses}敗' if draws == 0 else f'{wins}勝{draws}分{losses}敗'
    return ('<section><h2>過去の対戦</h2>'
            f'<p>直近の直接対決は{escape(a)}から見て'
            f'<strong>{record}</strong>（過去{len(pair)}試合）。</p>'
    ...
    for t in (m["home"], m["away"]):
        ...
        if e:
            rows += (f'<tr><td>{escape(t)}</td><td>{e["rank"]}位</td>'
                     f'<td>{e["points"]}</td><td>{e["wins"]}-{e["losses"]}</td>'
                     f'<td>{escape(str(e["goal_diff"]))}</td></tr>')
    if rows:
        body += ('<section><h2>両チームの今季成績</h2>'
                 '<div class="tbl"><table><thead><tr><th>チーム</th><th>順位</th><th>勝ち点</th>'
                 '<th>勝-敗</th><th>得失点差</th></tr></thead>'
...
def build_portal(leagues, articles, meta):
    ...
    hero_img = ('<img class="hero-img" src="assets/hero.jpg" alt="" width="1440" height="810">'
                if (ASSETS / "hero.jpg").exists() else "")
    body = ('<div class="hero">'
            f'{hero_img}'
            '<div class="hero-text">'
            '<p class="hero-kicker">関東の大学アメリカンフットボール</p>'
            '<h1>大学アメフトの試合結果・日程・順位を毎日更新</h1>'
            f'<p class="hero-sub">TOP8・BIG8 全{len(leagues)}カテゴリ・{total_teams}チームの結果・順位・過去の対戦データを掲載　|　最終更新 {escape(meta["fetched_at"][:10])}</p>'
            '</div>'
            '</div>')
    body += '<section><h2>関東の大学アメリカンフットボール</h2>'
    cards = ""
    for lg in leagues:
    ...
    write_page("", page(rel, f"{SITE_NAME} | 大学アメフトの試合結果・順位・データ", body, meta,
                        path="",
                        desc="関東の大学アメリカンフットボール（TOP8・BIG8）の試合結果・日程・順位表を毎日更新。過去の対戦データも掲載。"))
...
def build_league(lg, articles):
    ...
    if recent:
        body += ('<section><h2>最新の試合結果</h2>'
                 + match_rows_table(recent, L)
                 + f'<p class="more"><a class="cta" href="{L}schedule/index.html">全試合の日程・結果 →</a></p></section>')
    if upcoming:
        body += ('<section><h2>今後の試合</h2>'
                 + match_rows_table(upcoming, L) + '</section>')
    if awaiting:
        body += ('<section><h2>結果反映待ちの試合</h2>'
                 '<p class="note">連盟の公表データにまだスコアが入っていない日程（延期の可能性あり）。</p>'
                 + match_rows_table(awaiting, L) + '</section>')
    for block, entries in standings.items():
        if entries:
            body += f'<section><h2>順位表（{escape(block)}）</h2>' + standings_table(block, entries, L) + '</section>'
    write_page(code, page(R, f'{league_name} 試合結果・日程・順位表 | {SITE_NAME}', body, meta,
                          path=f"{code}/",
    ...
    if upcoming_all:
        body += ('<section><h2>今後の試合</h2>'
                 + match_rows_table(upcoming_all, L) + "</section>")
    body += ('<section><h2>試合結果</h2>'
             + match_rows_table(list(reversed(played)), L) + "</section>")
    if awaiting:
        body += ('<section><h2>結果反映待ちの試合</h2>'
                 + match_rows_table(awaiting, L) + "</section>")
    write_page(f"{code}/schedule",
               page(R, f'試合日程・結果 | {league_name} | {SITE_NAME}', body, meta,
    ...
    for block, entries in standings.items():
        if entries:
            if len(standings) > 1:
                body += f'<h2>{escape(block)}</h2>'
            body += standings_table(block, entries, L)
    body += '<p class="note">※順位・勝ち点は関東学生アメリカンフットボール連盟（KCFA）が公表する星取表の値をそのまま掲載しています。得失点差は編集部が試合結果から集計した参考値です。</p>'
    write_page(f"{code}/standings",
               page(R, f'順位表 | {league_name} | {SITE_NAME}', body, meta,
                    path=f"{code}/standings/", desc=f'{league_name}の順位表。勝ち点・得失点差を毎日更新。',
                    subnav=sub))
    ...
    write_page(f"{code}/teams",
               page(R, f'チーム一覧 | {league_name} | {SITE_NAME}', body, meta,
    ...
    for team, info in lg["teams"].items():
        ...
        if my_played:
            body += ('<section><h2>試合結果</h2>'
                     + match_rows_table(list(reversed(my_played)), L)
                     + "</section>")
        if my_upcoming:
            body += ('<section><h2>今後の日程</h2>'
                     + match_rows_table(my_upcoming, L)
                     + "</section>")
        season_rows = ""
        for h in lg["hist"]:
            # BIG8はAブロック/二次上位リーグ等、1年に複数区分の成績を持つことが
            # あるため区分名も表示する（区分が1つだけの年はリーグ名と同じになる）。
            year_hits = [(hblock, next((x for x in entries if x["team"] == team), None))
                         for hblock, entries in h["standings"].items()]
            year_hits = [(b, e) for b, e in year_hits if e]
            for hblock, e in year_hits:
                block_cell = f'（{escape(hblock)}）' if len(year_hits) > 1 else ""
                season_rows += (f'<tr><td>{h["year"]}年{block_cell}</td>'
                                f'<td>{e["rank"]}位</td>'
                                f'<td>{e["wins"]}-{e["losses"]}</td>'
        ...
        if season_rows:
            body += ('<section><h2>年度別成績</h2>'
                     ...
                     '<th>勝-敗</th><th>総得点-総失点</th></tr></thead>'
                     f'<tbody>{season_rows}</tbody></table></div>'
                     '<p class="note">※順位はKCFAが公表する星取表の値です。</p></section>')
        if articles:
            art_links = "".join(
    ...
    for m in matches:
        ...
        detail_rows = f'<tr><th>日付</th><td>{d}</td></tr>'
        if m["venue"]:
            detail_rows += f'<tr><th>会場</th><td>{escape(m["venue"])}</td></tr>'
        detail_rows += f'<tr><th>スコア</th><td>{score_str(m)}</td></tr>'
        body += f'<div class="tbl"><table class="detail"><tbody>{detail_rows}</tbody></table></div>'
        if m.get("pdf_url"):
            body += f'<p class="note"><a href="{escape(m["pdf_url"])}">公式結果PDF（KCFA）を見る →</a></p>'
        body += ('<p class="links">'
        ...
        write_page(f"{code}/matches/{m['id']}",
                   page(R, match_headline(m) + f' | {lg["label"]} | {SITE_NAME}', body, meta,
...
def build_records(lg):
    ...
    if rows:
        body += ('<section><h2>通算勝率ランキング（10試合以上）</h2>'
                 ...
                 '<th>勝-敗</th><th>勝率</th><th>総得点-総失点</th></tr></thead>'
                 f'<tbody>{rows}</tbody></table></div></section>')
        ...
        for block, entries in h["standings"].items():
            ...
            if top:
                ...
                rows += (f'<tr><td>{h["year"]}年</td><td>{escape(block)}</td><td>{name}</td>'
                         f'<td>{top["wins"]}-{top["losses"]}</td></tr>')
    if rows:
        body += ('<section><h2>年度別優勝</h2>'
                 '<div class="tbl"><table><thead><tr><th>年度</th><th>区分</th><th>チーム</th>'
                 f'<th>成績</th></tr></thead><tbody>{rows}</tbody></table></div>'
                 '<p class="note">※KCFA（関東学生アメリカンフットボール連盟）が公表する星取表の値です。'
                 f'最新情報は<a href="{escape(meta["source_url"])}">{escape(meta["source"])}</a>をご確認ください。</p></section>')
    ...
    if rows:
        body += ('<section><h2>今シーズンの首位（進行中）</h2>'
                 '<div class="tbl"><table><thead><tr><th>区分</th><th>首位</th><th>勝ち点</th></tr></thead>'
                 f'<tbody>{rows}</tbody></table></div></section>')

    write_page(f"{code}/records",
               page(R, f'記録室（歴代記録・通算成績） | {lg["label"]} | {SITE_NAME}', body, meta,
                    path=f"{code}/records/",
                    desc=f'{meta["league"]}の歴代記録。最多得点試合、通算勝率ランキング、年度別優勝をデータから集計。',
                    subnav=sub))
...
def build_articles(articles, meta):
    ...
    body = ('<h1>読みもの</h1>'
            '<p class="lead">戦術・練習・チーム運営・分析 ― 大学アメフトの現場で使える知見をまとめています。</p>'
            f'<div class="digest">{cards}</div>')
    write_page("articles",
               page(rel, f"読みもの（戦術・練習・チーム運営・分析） | {SITE_NAME}", body, meta,
                    path="articles/",
                    desc="大学アメリカンフットボールの戦術・練習メニュー・チーム運営の実践的なノウハウ記事。"))
    rel = "../../"
    for a in articles:
...
def build_glossary(meta):
    ...
    body = ('<h1>アメフト用語辞典</h1>'
            f'<p class="lead">試合観戦や部活動で使われるアメフト用語{len(terms)}語を分野別にまとめました。</p>')
...
def build_videos(meta):
    ...
    body = ('<h1>動画インデックス</h1>'
            '<p class="lead">大学アメフトの試合映像・配信を公式ソースから探せるリンク集です。</p>')
    for cat, items in cats.items():
        cards = "".join(
    ...
    write_page("videos",
               page(rel, f"動画インデックス（試合映像・ライブ配信） | {SITE_NAME}", body, meta,
                    path="videos/",
                    desc="大学アメリカンフットボールの試合映像・ライブ配信を公式ソースから探せるリンク集。"))


DASHBOARD_PATH = "dash-am-ops"  # 非公開運用ダッシュボード（noindex・sitemap非掲載）
...
def build_dashboard(leagues, articles, meta):
    ...
    recent_results_n = sum(1 for lg in leagues for m in lg["matches"]
                           if m["status"] == "played" and m["date"] and m["date"] >= week_ago)

    body = ('<h1>運営ダッシュボード</h1>'
            f'<p class="lead">{SITE_NAME}の定点観測。毎朝の自動更新で最新化されます。'
    ...
    body += ('<section><h2>サイト全体</h2><div class="stat-row">'
             ...
             f'<div class="stat"><span class="num">{recent_results_n}</span>直近7日の結果</div>'
             '</div></section>')
    ...
    body += ('<section><h2>外部ツール（クリックで開く）</h2><ul>'
             ...
             f'<li><a href="{SOURCE_URL_FOR_DASH}">KCFA公式サイト</a></li>'
             '</ul></section>')
...
SOURCE_URL_FOR_DASH = "https://www.kcfa.jp/"
...
<rect width="64" height="64" rx="14" fill="#0e1f3d"/>
<rect x="10" y="40" width="44" height="8" rx="4" fill="#c8902e"/>
<text x="32" y="36" font-family="Arial, sans-serif" font-size="24" font-weight="bold"
 fill="#ffffff" text-anchor="middle">AM</text>
</svg>
"""

# アクセントカラーのみ rugbymania（赤系 #c8442e）から変更（ゴールド系 #c8902e）。
# それ以外のCSS構造・クラス名はrugbymaniaのSTYLEを踏襲。
STYLE = """
:root {
  --navy:#0e1f3d; --navy-2:#1e3c66; --accent:#c8902e; --accent-dark:#96690f;
  --accent-soft:#faf1de; --ink:#0f1f33; --sub:#5b6b7b; --line:#dfe5ec;
...
.brand-sub { font-size:.6rem; color:var(--accent-dark); font-weight:700; letter-spacing:.15em;
...
.tbl { overflow-x:auto; background:var(--surface); border:1px solid var(--line);
  border-radius:12px; box-shadow:0 1px 3px rgba(14,31,61,.06); }
...
.stat { background:var(--surface); border:1px solid var(--line); border-radius:12px;
  ...
  text-align:center; box-shadow:0 1px 3px rgba(14,31,61,.06); }
...
.report { background:var(--surface); border:1px solid var(--line);
  ...
  box-shadow:0 1px 3px rgba(14,31,61,.06); }
...
.digest-card { background:var(--surface); border:1px solid var(--line);
  border-radius:12px; padding:.9rem 1rem 1rem; box-shadow:0 1px 3px rgba(14,31,61,.06); }
...
.article { background:var(--surface); border:1px solid var(--line); border-radius:12px;
  padding:1.4rem 1.6rem 1.6rem; box-shadow:0 1px 3px rgba(14,31,61,.06); }
...
def main():
    ...
    if not leagues:
        raise SystemExit("リーグデータがありません（fetch_amefoot_kanto.pyを先に実行）")
...
# ---------------------------------------------------------------------------
# rugbymania (C:\Users\tatsu\claudecode\rugbymania) の pipeline/generate_site.py
# からの主な変更点:
...
#    星取表の勝敗記号（○●）から書き出す winner フィールドを使う
#    （winner_team()）。タイブレークで決着した同点カード（例:
#    2025年度TOP8 立教大-東京大 17-17、東京大がタイブレーク15-13で勝利）を
#    正しく扱うための変更。
#  - 試合に「時間」情報がない（kcfa.jpの星取表・月別日程ページとも試合時刻を
#    掲載していない）ため、日程表から時間列を削除。
#  - 会場情報は星取表からは取得できない（PDFリンクのファイル名から日付のみ
#    判明する）ため、venueが空の試合は日程表・試合詳細で会場列ごと省略する。
#  - 試合詳細ページにKCFA公式結果PDFへのリンクを追加（pdf_url）。
#  - 順位表・得失点差の注記を「編集部による参考値の自前集計」から「KCFA公表値
#    をそのまま掲載（得失点差のみ試合結果からの集計）」に変更。
#  - GA4測定ID・Search Console確認トークンは未発行のため空欄
#    （page()内のgtag挿入コードはコメントアウトのまま用意）。
